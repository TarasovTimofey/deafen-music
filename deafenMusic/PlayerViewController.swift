...
//  myMusic
//
//  Created by Тагир Файрушин on 06.07.2024.
//
...
class PlayerViewController: UIViewController {
    ...
    // Timer for updating scrollSlider
    ...
    // Labels for current and remaining time
    ...
    func configure(){
        ...
        updateTimer?.invalidate() // Invalidate any existing timer // NEW
        updateTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { [weak self] timer in
            if let self = self, let player = self.player, player.isPlaying {
                ...
                // Проверяем, завершился ли текущий трек
                if player.currentTime + 1.0 >= player.duration {
                    // Трек завершился, выполняем необходимые действия
                    self.position += 1
                    player.stop()
                    for subview in self.holder.subviews {
                        subview.removeFromSuperview()
                    }
                    self.configure() // Перезапускаем конфигурацию для следующего трека
                    player.play()
                    // Отменяем таймер, так как мы больше не будем обновлять UI для старого трека
                    timer.invalidate()
                }
            } else {
                timer.invalidate()
